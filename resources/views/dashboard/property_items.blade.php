...
@section('title') {{awtTrans('خصائص السمة')}} @endsection
@section('style')
            ...
            <div class="card-header">
                <div class="col-12">
                    <div class="btns header-buttons">
                        <button class="btn bg-teal" title="{{awtTrans('اضافة')}}" data-toggle="modal" data-target="#add-property_item">
                            <i class="fas fa-plus"></i>
                        </button>
                        <button class="btn btn-info" title="{{awtTrans('السمات')}}" onclick="back()">
                            ...
                            @foreach ($data as $i=>$item)
                                <tr style="text-align: center">
                                    ...
                                    <div class="btn-action">
                                        <a href="#" class="btn btn-sm bg-teal" title="{{awtTrans('تعديل')}}" onclick="editSection({{ $item }})"  data-toggle="modal" data-target="#edit-property_item">
...
@section('modal')
    <!-- add-property_item modal-->
    <div class="modal fade" id="add-property_item" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{awtTrans('إضافة خاصية')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('addproperty_item')}}" id="addSectionForm" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="property_id" value="{{$id}}">

                <div class="form-group non-colorDiv">
                ...
                <div class="form-group colorDiv" @if($id != 2) style="display: none" @endif>
                    <label>{{awtTrans('اللون')}}</label>
    ...
    <!--end add-property_item modal-->
    <!-- edit-property_item modal-->
    <div class="modal fade" id="edit-property_item" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{awtTrans('تعديل خاصية')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="{{route('updateproperty_item')}}" id="updateSectionForm" method="POST" enctype="multipart/form-data">
                @csrf
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form action="{{route('deleteproperty_item')}}" method="post" class="d-flex align-items-center">
                @csrf
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form action="{{route('deleteproperty_items')}}" method="post" class="d-flex align-items-center">
                @csrf
                <input type="hidden" name="property_item_ids" id="delete_ids">
...
@section('script')
    <script>
        ...
        function back() {
            window.location.assign("{{route('propertys')}}");
        }
        ...
        function editSection(property_item){
            $('#edit_id').val(property_item.id);
            $('#edit_title_ar').val(property_item.title_ar);
            $('#edit_title_en').val(property_item.title_en);
            $('#edit_color').val(property_item.color);
        }
        ...
        function checkIds(){
            let property_itemsIds = '';
			$('.checkSingle:checked').each(function () {
				let id = $(this).attr('id');
				property_itemsIds += id + ' ';
			});
			let requestData = property_itemsIds.split(' ');
